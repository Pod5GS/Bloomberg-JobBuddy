...
ffm_float ffm_predict(ffm_node *begin, ffm_node *end, ffm_model *model)
{
    ...
    ffm_long align1 = (ffm_long)model->m*align0;
    cout << "******** align0: " << align0 << endl;
    cout << "******** align1: " << align1 << endl;

    ffm_float t = 0;
    for(ffm_node *N1 = begin; N1 != end; N1++)
    {

	cout << "N1: " << begin << "end: " << end << endl;
	ffm_int j1 = N1->j;
        ffm_int f1 = N1->f;
        ffm_float v1 = N1->v;
	cout << "j1: " << j1 << ", f1: " << f1 << ", v1: " << v1 << endl;
        if(j1 >= model->n || f1 >= model->m)
            continue;
        ...
        for(ffm_node *N2 = N1+1; N2 != end; N2++)
        {
            ...
            ffm_int f2 = N2->f;
            ffm_float v2 = N2->v;
	    cout << "j2: " << j2 << ", f2: " << f2 << ", v2: " << v2 << endl;
            if(j2 >= model->n || f2 >= model->m)
                continue;

            ffm_float *w1 = model->W + j1*align1 + f2*align0;
            ffm_float *w2 = model->W + j2*align1 + f1*align0;
	    cout << "model->W: " << model->W << ", w1: " << *w1 << ", w2: " << *w2 << endl;

            ffm_float v = v1*v2*r;

            for(ffm_int d = 0; d < model->k; d++)
	    {
		cout << "w1[d]: " << w1[d] << ", w2[d]: " << w2[d] << endl;
                t += w1[d]*w2[d]*v;
	    }
	    cout << "t: " << t << endl;
        }
